...
#recibe vector de tiempos y señal
#devuelve un vector con los tiempos de transicion; intervalos entre transciones
def LookForTransitions (t, signal):
    last_edge = 0
    transition = np.zeros(100)
    accum_time = 0
    tindex = 0
...
#hago un append de vectores
vapp = np.zeros(signal.size + signaln.size)
vapp [0:signal.size] = signal
vapp [signal.size:vapp.size] = signaln
...
#tr = LookForTransitions(t, signaln)
tr = LookForTransitions(t2, vapp)
print tr
