...
"""
        ...
        Comparo resultados PID con Custom, aca solo comparo para 
        determinar los valores ver direct_digital_design_01
...
controller_pid_tf = TransferFunction(b_pid, a_pid, dt=td)
...
print ("Digital PID Controller:")
print (controller_pid_tf)
...
contr_pid_dig = lti_to_sympy(controller_pid_tf)
...
ol_pid_dig = plant_dig * contr_pid_dig
open_loop_pid_dig = sympy_to_lti(ol_pid_dig)
close_loop_pid_dig = sympy_to_lti(ol_pid_dig/(1+ol_pid_dig))
...
open_loop_pid_dig = TransferFunction(open_loop_pid_dig.num, open_loop_pid_dig.den, dt=td)    
close_loop_pid_dig = TransferFunction(close_loop_pid_dig.num, close_loop_pid_dig.den, dt=td)
...
tout, yout_pid_zoh = dstep([close_loop_pid_dig.num, close_loop_pid_dig.den, td], t=t)
yout1 = np.transpose(yout_pid_zoh)
yout0 = yout1[0]
yout_pid_zoh = yout0[:tout.size]
...
if Escalon_CloseLoop_Original_Digital == True:
    fig, ax = plt.subplots()
    ax.set_title('Step CloseLoop Custom Orange PID Cyan Original ZOH Yellow')
    ax.set_ylabel('Output')
    ax.set_xlabel('Tiempo [s]')
    ax.grid()
    ax.plot(tout, yout_planta_dig_zoh, 'y')
    ax.plot(tout, yout_pid_zoh, 'c')    
    ax.plot(tout, yout_zoh, color='orange')
